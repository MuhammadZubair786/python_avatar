...
class DeitHistoryUser extends StatefulWidget {
  int index;

   DeitHistoryUser({Key? key,required this.index}) : super(key: key);

  @override
  _DeitHistoryUserState createState() => _DeitHistoryUserState();
}

class _DeitHistoryUserState extends State<DeitHistoryUser> {
          ...
          children: [
            ...
            Expanded(
              child: GetBuilder<GlobalController>(
                builder: (controller) => controller.AllDiet.isEmpty
                    ? Center(
                        child: CircularProgressIndicator(),
                      )
                    : ListView.builder(
                        shrinkWrap: true,
                        itemCount: controller.AllDiet[0][widget.index]['dietEntries'].length,
                        itemBuilder: (context, index) {
                          ...
                          return GestureDetector(
                            onTap: () {
                              print(controller.AllDiet[0][widget.index]);
                              Get.to(() => NewDietPlanScreen2(
                                    userDietDataList: controller.AllDiet[0][widget.index]['dietEntries'][index]['mealEntries'],
                                    day: controller.AllDiet[0][widget.index]['dietEntries'][index]['day'].toString(),
                                  ));
                            },
